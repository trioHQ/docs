...
This guide explains the onboarding flow for an integration in which an organization acts as an **orchestrator** for its customers' organizations.

The process allows the orchestrator organization to submit the data required to open the customer's account, track the Compliance analysis through webhooks and, once approved, perform operations on behalf of the customer's organization.
...
- **Orchestrator organization:** the organization that holds the API credentials and controls the flow for its customers.
...
The orchestrator organization continues to use its own credentials. To perform an operation on a customer's organization, simply provide the target organization's identifier in the `Trio-Target-Org-Id` header.
...
Authentication uses **Basic Authentication**, with the orchestrator organization's `client_id` and `client_secret`.
...
The `liveness_link` is made available to the representative during the onboarding flow and can also be received in the `entity.in_review` webhook, when applicable.
...
The orchestrator organization **does not need to keep polling the API** to find out whether the analysis has finished.
...
Once onboarding has been approved, the orchestrator organization can make calls on behalf of the customer's organization.

To do this, use the same credentials as the orchestrator organization and provide the customer's `org_id` in the header:
...
Access is validated by Trio on every request. If the orchestrator organization does not have access to the specified organization, the API returns `403`.
...
1. Customer requests account opening
              |
              v
2. Orchestrator collects the data
              |
              v
3. POST /onboarding
              |
